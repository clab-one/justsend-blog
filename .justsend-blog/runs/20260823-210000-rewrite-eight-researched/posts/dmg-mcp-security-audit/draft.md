...
이 위협 모델의 장점은 코드 공개 여부와 무관하다는 점이다. 공격자가 tool schema와 queue row를 안다는 가정에서 test를 쓰면 문서의 “내부용” 표지가 authorization을 대신하지 못한다. 배포 산출물은 공개되고 protocol은 관측된다. 남아야 하는 것은 검증 가능한 identity와 최소 권한이다.
