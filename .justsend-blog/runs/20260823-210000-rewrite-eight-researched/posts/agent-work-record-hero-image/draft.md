...
기존 글은 결과를 1348자에 요약했다. corpus 깊이 기준에 못 미쳤고, 왜 renderer가 그림 내용을 결정하면 안 되는지와 `jsattach` 중복이 어떻게 생겼는지, running helper를 교체하다 연결을 끊은 실패가 빠졌다. 이번 글은 image를 장식이 아니라 기록 데이터의 한 종류로 다룬다.
<!-- evidence: JS-E007 -->
...
이 구조는 work record를 CI log와 다르게 만든다. log는 실행 순서가 중심이지만 record는 사용자가 나중에 결과를 찾는 문서다. 실패와 dead end는 note에 남겨 다음 agent가 반복하지 않게 하고, main body는 최신 결론을 보여 준다.
<!-- evidence: JS-E001 -->
